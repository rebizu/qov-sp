...
| Phase 3.9 structured P-frames (v3.9) | DONE 2026-09-16 — flag 0x04, P bytes −43.8% at q60, spec §3.4.6, all impls + corpus |
| Bandwidth exploration (BANDWIDTH-EXPLORATION.md) | DONE 2026-09-17 — knobs ADOPTED as spec v3.11 (AC dead-zone 1.0 + block-skip slope 16: cam720 q60 video 260.1 → 197.9 kbps, −23.9% at SSIM −0.0082, corpus regen + conformance GREEN ×3); 160×120 bottom rung ADOPTED in the TS/C# adaptation ladders + call demo (stage 2; deficit-persist engagement, bottom-first recovery, unit tests both stacks); rate control measured (regulates, no fixture win — keep as qov_set_quality plumbing); frame skip closed (0.2% ceiling). Exposed + fixed pre-existing C# partial-block padding defect (cs_encode parity on odd_dims) and a stale v2-version assertion in EncoderTests |
| Phase 4 v4 subtraction/freeze | NOT STARTED |

Conformance state: GREEN, 46 cases × 8 steps, expected_failures.json empty (verified
across all three implementations on Windows and Linux).
Spec version: 3.9. Remotes: push target is `rebizu` (github.com/rebizu/qov-sp), branch `master`.
