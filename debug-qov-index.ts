...
interface FrameAnalysis {
  frameNumber: number;
  frameType: 'I-frame' | 'P-frame';
  indexErrors: Array<{
    plane: 'Y' | 'U' | 'V';
    pixelIndex: number;
    indexSlot: number;
    totalPixelsDecoded: number;
  }>;
  indexTableSnapshot: {
    Y: number[];
    U: number[];
...
class QOVAnalyzer {
  ...
  private prevY: Uint8Array | null = null;
  private prevU: Uint8Array | null = null;
  private prevV: Uint8Array | null = null;
  ...
  analyze(maxFrames: number = 20): FrameAnalysis[] {
    const results: FrameAnalysis[] = [];

    // Read header
    const magic = String.fromCharCode(this.readU8(), this.readU8(), this.readU8());
    if (magic !== 'QOV') {
      throw new Error('Invalid QOV file');
    }
  ...
  private decodeYuvPlaneKeyframe(plane: Uint8Array, index: number[], size: number): void {
    let px = 0;
    let prevVal = 128;
