 ...
 */
export function rgbaToYuv420Planes(pixels, width, height, hasAlpha) {
 ...
 */
export function yuv420PlanesToRgba(yPlane, uPlane, vPlane, aPlane, width, height) {
 ...
 */
export function rgbaToYuv422Planes(pixels, width, height, hasAlpha) {
    const yPlane = new Uint8Array(width * height);
    const uvWidth = Math.ceil(width / 2);
 ...
 */
export function yuv422PlanesToRgba(yPlane, uPlane, vPlane, aPlane, width, height) {
    const pixels = new Uint8ClampedArray(width * height * 4);
    const uvWidth = Math.ceil(width / 2);
    for (let py = 0; py < height; py++) {
        for (let px = 0; px < width; px++) {
 ...
 */
export function rgbaToYuv444Planes(pixels, width, height, hasAlpha) {
 ...
 */
export function yuv444PlanesToRgba(yPlane, uPlane, vPlane, aPlane, width, height) {
    const size = width * height;
