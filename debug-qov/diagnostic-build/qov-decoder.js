...
export class QovDecoder {
    constructor(data) {
        ...
        this.prevFrame = null;
        this.currFrame = null;
        this.use32BitChunkSize = false; // true for version 0x02+
    ...
    decodeYuvPlaneKeyframe(plane, size) {
        let prevVal = 0;
        const index = new Array(64).fill(-1); // Initialize to -1 to avoid false matches with value 0
        let px = 0;
        while (px < size) {
            const b1 = this.readU8();
            if ((b1 & 0xc0) === 0xc0 && b1 < 0xfe) {
                ...
                if (prevVal === -1) {
                    ...
                    prevVal = 0; // Fallback to avoid artifacts
                }
                plane[px++] = prevVal;
        ...
        while (px < size) {
            ...
            if (b1 === 0x00) {
                ...
                if (index[idx] === -1) {
                    ...
                    index[idx] = 0; // Fallback to avoid artifacts
                }
                plane[px++] = index[idx];
                // BUG FIX: Update index table after using INDEX, to match encoder behavior
                // The encoder updates index[idx] = val after emitting INDEX (encoder line 504)
                // Decoder must do the same to keep tables in sync
