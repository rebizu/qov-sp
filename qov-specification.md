...
All chunks share a common 8-byte header:
...
2       2     chunk_size   Size of data after header, big-endian
...
0x00-0x3F    QOV_YUV_INDEX   | 00 | index (6 bits) |
                             Index into 64-entry YUV cache
                             Hash: (y*3 + u*5 + v*7) % 64

0x40-0x7F    QOV_YUV_DIFF    | 01 | dy (4) | du (1) | dv (1) |
                             dy bias: 8 (range -8 to +7)
                             du/dv bias: 1 (range -1 to 0)

0x80-0xBF    QOV_YUV_LUMA    | 10 | dy (6 bits) |
                             | du (4 bits) | dv (4 bits) |
                             dy bias: 32, du/dv bias: 8

0xC0-0xFD    QOV_YUV_RUN     | 11 | run (6 bits) |
                             Repeat previous pixel 1-62 times

0xFE         QOV_YUV_FULL    | 11111110 | y | u | v |
                             Literal YUV values (3 bytes)

0xFF         QOV_YUV_FULLA   | 11111111 | y | u | v | a |
                             Literal YUVA values (4 bytes)
```

### 3.3 Temporal Opcodes (P-frames and B-frames)

Additional opcodes for inter-frame prediction:
...
### 4.1 YUV 4:2:0 Plane Encoding

For YUV420 colorspace, planes are encoded separately:

```
1. Y plane  - Full resolution (width × height pixels)
2. U plane  - Quarter resolution ((width/2) × (height/2) pixels)
3. V plane  - Quarter resolution ((width/2) × (height/2) pixels)
4. A plane  - Full resolution (only if YUVA420)
```

Each plane uses its own running index cache.
...
Data (no motion vectors):
  [Pixel data using temporal + standard opcodes]
  [8-byte end marker]
